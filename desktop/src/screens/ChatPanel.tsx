    ...
    >
      ...
      {showInfo && (
        <div style={{
          ...
          <div>Delivered: {msg.delivered ? 'Yes' : 'No'}</div>
          <div>Read: {msg.read ? 'Yes' : 'No'}</div>
...
export default function ChatPanel({ token, myUsername, partner, partnerOnline, masterToken, onMasterTokenSaved, onCall, onJoinMeeting, onBack }: Props) {
  ...
  async function handleReact(messageId: number, emoji: string) {
    try {
      ...
      // silently fail — reaction toggle isn't worth surfacing an error toast for
    }
  }
  ...
  async function handleSend() {
    ...
    if (!editTarget) setPending(prev => [...prev, pendingMsg]);
